...
// Database Connection
mongoose.connect("mongodb+srv://devwish29:VZ1LVat74qR5P4di@cluster0.ysdy3.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0", {
...
app.post("/submit", async (req, res) => {
  ...
  const uniqueId = (count + 1).toString().padStart(5, "0");
...
app.get("/get/:uniqueId", async (req, res) => {
  ...
  if (!imageObject) return res.status(500).json({ message: "Object not found" });
...
app.put("/update/:uniqueId", async (req, res) => {
  ...
  if (!imageObject) return res.status(404).json({ message: "Object not found" });
